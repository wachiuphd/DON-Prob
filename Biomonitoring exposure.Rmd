...
### Portuguese population random sample
Martins et al. (2019)
...
### Chinese population random sample
Wang et al. 2019
...
for (j in 1:length(Chi_ue)) {
  ...
  #for (k in 1:npop) {
    z_i <- zrand[j]
    AED <- (Chi_ue[j] * AF_interTD * (Intra.GSD^z_i))/(Cavg_dose_a*U_Cavg_h.GM)
...
### European population random sample; serum
Ruyck et al. 2020
...
yran.df <- cbind(yranpop, ymean, y1perc, ymed, y99perc)
write.csv(yran.df, file="Perc European population serum fraction of response.csv", row.names=FALSE)
```
...
#quantile (Portuguese population)
...
yran.df.euserum <- read.csv("Perc European population serum fraction of response.csv")
...
expboxplot.port <- ggplot(melt.yran.port, aes(x=value, y=variable))+
  geom_boxplot(aes(fill=variable),
               outlier.shape=NA,
               show.legend = FALSE)+
  scale_x_continuous(limits = c(-0.15,0), labels = c("-15%","-10%","-5%","0"))+
...
boxplot_frac <- ggarrange(expboxplot.port, expboxplot.chi, expboxplot.euserum, ncol=3, nrow=1,
                          labels = c("A","B","C"), font.label = list(size = 18))
ggsave(boxplot_frac, file="Boxplot fraction of response.pdf", width=24, height=10, scale=0.6)
```
