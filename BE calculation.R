...
BE.urine.df <- data.frame(bmd, Cavg_dose_a, GM_U_Cavg, U_Cavg_h.GM,
                    AF_interTD,GSD_U_Cavg, TK_var.Cavg_h,
                    TD_var.GSD, AF_intraTKTD.be, BE.urine)
fwrite(BE.urine.df,"Urine BE.samples.csv")

#### Blood BE
BE.blood <- (bmd*Cavg_dose_a) / (AF_interTD*TD_var.GSD)
cat("BE.blood",quantile(BE.blood,prob=c(0.5,0.05,0.95)),"\n")

BE.blood.df <- data.frame(bmd, Cavg_dose_a, AF_interTD,
                          TD_var.GSD, BE.blood)
